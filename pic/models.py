...
class Company(models.Model):
    ...
    companytype = models.CharField(verbose_name='公司类型', choices=(
        ('供应商', '供应商'), ('客户', '客户'), ('其它', '其它')), max_length=10, default="供应商")
    create_time = models.DateTimeField(verbose_name='创建时间', auto_now_add=True)
    create_date = models.DateField(verbose_name='创建日期', auto_now_add=True)
    status = models.CharField(verbose_name='状态', choices=(
        ('正式', '正式'), ('作废', '作废')), max_length=10, default="正式")
...
class Pic(models.Model):
    ...
    ziplink = models.FileField(
        verbose_name='zip文档链接',
        upload_to='photos/%Y/%m/%d',
        storage=ImageStorage())
    pdflink = models.FileField(
        verbose_name='pdf文档链接',
        blank=True,
        null=True,
        upload_to='photos/%Y/%m/%d')
